...
public final class JceInputStream
{
    ...
    public String readString(int i, boolean z)
    {
        if (this.skipToTag(i))
        {
            ...
            else if (z)
            {
                ...
                try
                {
                    return new String(b, this.sServerEncoding);
                }
                catch (UnsupportedEncodingException e)
                {
                    return new String(b);
            ...
            {
                int r0 = this.bs.getInt();
                if (r0 > 104857600 || r0 < 0 || r0 > this.bs.capacity())
                {
                    throw new JceDecodeException("String too long: " + r0);
                }
                byte[] b = new byte[r0];
                this.bs.get(b);
                try
    ...
    private <K, V> Map<K, V> readMap(Map<K, V> mr, Map<K, V> m, int tag, boolean isRequire)
    {
        ...
        Map.Entry<K, V> en = (Map.Entry)it.next();
        K mk = en.getKey();
        V mv = en.getValue();
        if (skipToTag(tag))
        {
            ...
            switch (hd.type)
            {
                case 8:
                    ...
                    for (int i = 0; i < size; i++)
                    {
                        K k = (K) readobj(mk, 0, true);
                        V v = (V) readobj(mv, 1, true);
                        mr.put(k, v);
                    }
                    break;
                default:
    ...
    public JceStruct readunnew(JceStruct o, int tag, boolean isRequire)
    {
        JceStruct ref = null;
        if (skipToTag(tag))
        {
    ...
    public <T> Object readobj(T o, int tag, boolean isRequire)
    {
